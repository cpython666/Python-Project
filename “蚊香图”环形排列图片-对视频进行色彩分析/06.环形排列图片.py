import os
image_files = [f for f in os.listdir('frames_')]
...
desired_total = img_num
first_term = n
common_difference = n_step
layer = 0
total = 0
# 逐步增加n，直到总和超过500为止
while total + first_term <= desired_total:
    total += first_term
    first_term += common_difference
    layer += 1
# 输出结果
print(f"可以堆叠{layer}层，总共拥有{total}张图片，总和小于500。")
x=layer
...
for i in range(x):
    ...
    for j in range(n_tmp):
        ...
        # 打开图片文件或创建一个新图像
        sub_image = Image.open(os.path.join('frames_',image_files[img_cur]))  # 在这里创建一个新图像，或者打开你的图片文件
        img_cur+=1
